...
public class Blackjack {
	...
	public static void main(String[] args) {
		...
		// Stores the difference between how many games the player wins and how many games the dealer wins
		double difference = 0;
		...
		for (int i = 0; i < iterations; i++) {
			...
			int[] trueCardsLeft = { 16*decks,4*decks,4*decks,4*decks,4*decks,4*decks,
					4*decks,4*decks,4*decks,4*decks };
			...
			// If it is an ace and the player does not have an ace. If the player were to have
			...
			// Stores whether either player got Blackjack, when their first two cards add up to 21. The
			...
			// If, after drawing their first two cards, the player's total is 21, they have Blackjack
			if (playerTotal == 21) {
				blackjackFound = true;
			...
			if (dealerFirst + dealerSecond == 21) {
				...
				// If the player has Blackjack and the dealer does not. In traditional Blackjack, the 
			...
			// a boolean which represents whether the player should surrender, conceding the game right now
			// to only effectively lose half of a game. 
			...
			if (splittable) {
				...
				// If both of the player's cards are aces, then the total goes to 22 for the purpose of 
				// the input for the split method being 11, as the totals are divided by two.
				...
				// hitArray corresponds to whichever number is higher.
			...
			// Stores whether the player doubled, whether they can double their hand, and whether their 
			// hand busted, meaning its value went over 21. The hand has not yet doubled or busted, but 
			// it is doublable while there are only two cards in the player's hand.
			boolean doubled = false;
			boolean doublable = true;
			boolean playerBusted = false;
			
			// If the player total was set to 22 due to two aces being present and the hand was not split, 
			// it is set back to 12 as the second ace counts as one.
			if (playerTotal == 22) {
				playerTotal = 12;
			}
			
			// If the player should surrender, they effectively lose half the game and concede it, 
			...
			do {
				...
				if ((boolean) hitArray[0]) {
					...
					// If the total is greater than 21 and the player has an ace, the total is decreased 
					// by 10; the ace now counts as one, not 11
			...
			// The loop runs again if no reason has yet been found for the player not to hit again
			} while (playerHitting); 
			...
			while (dealerHitting) {
				// If the player busted, there is no need to check what the dealer has, as the player
				// automatically loses
				if (playerBusted) {
					break;
				}

				// If the dealer's total is greater than 16, then the dealer does not hit and the loop is 
				// broken out of
				if (dealerTotal > 16) {
					break;
				}
				...
				// If the dealer total goes over 21 and they have an ace, 10 is taken off as the dealer's
				// ace now counts as 1 rather than 11
				if (dealerTotal > 21 && dealerHasAce) {
					dealerTotal -= 10;
					dealerHasAce = false;
				} 
				
				// The dealer stops hitting if they have 17 or more, and they busted and automatically 
				// lose if their total is over 21
				if (dealerTotal > 16) {
					dealerHitting = false;
					if (dealerTotal > 21) {
						dealerBusted = true;
			...
			// Stores the result of the game in terms of how many the player won in comparison to the 
			// dealer
			int result = 0;
			...
			// Otherwise, if the player's total is greater than the dealer's, the player wins
	...
	/* Will run two hands against a dealer after the player has split their original hand, which they can
	 * do if their first two cards have the same value. Takes in as parameters the player's current value 
	 * in each hand (these are the same), the dealer's first card, whether the dealer has an ace, the 
	 * array of cards remaining according to the player, and the actual cards remaining, when considering
	 * the dealer's second card. This method will return a double which stores the amount of games the 
	 * player went up through these two hands. Each hand has the same value as the original hand, meaning 
	 * that the total amount of winnable points has been doubled, if this method is called. */
	public static double split(int num, int dealerFirst, int dealerSecond, boolean dealerHasAce, 
			int[] cardsLeft, int[] trueCardsLeft) {
		
		// Stores the number which will be returned.
		double result = 0;
		
		// Creates the array which stores the final totals of both of the player's hands.
		int[] arr = new int[2];
		
		// Gets the values for the next cards which will be dealt to each of the player's hands.
		...
		// Gets the totals of each of the player's hands after they've received their second card
		...
		for (int i = 0; i < 2; i++) {
			...
			// Stores the player's current value in the hand that is currently being played through
			int value = (i == 0 ? num1 : num2);

			// If the player started with an ace and then drew an ace, their total would go to 22, which
			// of course cannot happen, so one of the aces now counts as 1 and the player's hand value
			// goes to 12
			if (value == 22) {
				value = 12;
			}
			
			// A rule of Blackjack, or the variation that this program simulates, does not allow the 
			...
			do {
				...
				if ((boolean)hitArray[0]) {
					...
					// If the player should double the value of the game and they can double it, which is
					// when this is the first card they are receiving after splitting. They will double
					// in this case
					if ((boolean)hitArray[1] && doublable) {
						
						// The player cannot receive more than one card after splitting, meaning they are
						// no longer hitting
						playerHitting = false;
					...
					// If the player has an ace and their value went over 21, the ace will now count as
					// 1
					...
					// while having an ace is hitting again the optimal decision. So, we simply do not
					// bother checking what the player's best decision is for the sake of efficiency
	...
	/* Returns the sum of all the values in an integer array. Used for the purpose of finding the 
	 * probability that the dealer or player ends with a certain number, as the return value of this 
	 * method will be the denominator in such fractions throughout other methods in this class. */
	...
	/* Returns the value of a number as it corresponds to the amount of cards left in the shoe. Essentially, 
	 * all of the remaining cards are lined up, starting with tens/jacks/queens/kings, then going to aces,
	 * then twos, and so on. This method will get the value of the card at the location corresponding to the
	 * randomInt parameter. */
	private static int getVal(int randomInt, int[] cardsLeft) {
		...
		// Loops through the cardsLeft array, adding to the tracker how many cards of each number there are
